...
## v2.4.0: 2022/05/01
- 彻底去除在`settings.py`中添加用户信息和资源路径的方式，转移到`conf/user_config.ini`中
- 更新了sep新的登录方式（现在需要加密）
- 修复了资源同步存在的一些问题
- 资源存储目录设置默认存储为根目录`resources`下
- 添加了OCR自动识别验证码
> OCR有时候会识别错误，显示登录失败，重试几次即可

## v2.3.4: 2021/04/14
- 修复因课程网站变动导致的登录失效问题
- 新的登录接口还没做验证码的自动识别，需要人工手动输入验证码

## 旧版本信息
- [2.3.3] 添加不建议自动更新的说明，clone深度调整为1
- [2.3.1] 修复自动更新的一些问题，自动获取最新版本号，之前版本因为错误的更新代码无法兼容本次更新，建议重新`clone`项目
